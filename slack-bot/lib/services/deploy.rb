...
PLUGIN_CMD = <<~SH.freeze
  ...
  cd ../plugin-base && yarn build && cd ../plugiamo && cp ../plugin-base/dist/plugin-base.js node_modules/plugin-base/dist/ && \
...
CONSOLE_FRONTEND_CMD = <<~SH.freeze
  ...
  yarn install --silent --no-progress && \
  cd ../plugin-base && yarn build && cd ../console-frontend && cp ../plugin-base/dist/plugin-base.js node_modules/plugin-base/dist/ && \
