...
const getSlideName = (customerJourneySliderContent, index) => {
  return customerJourneySliderContent.edges[index].node.heading
}

const CustomPagingFactory = (customerJourneySliderContent, currentSlideIndex) => {
  const CustomPaging = index => (
    <SlideLabelContainer isSelected={currentSlideIndex === index}>
      <ImagesContainer>
        <IconContainer isLastElement={index === customerJourneySliderContent.edges.length - 1}>
          <img
            alt=""
            src={
              currentSlideIndex === index
                ? customerJourneySliderContent.edges[index].node.activeIcon.file.url
                : customerJourneySliderContent.edges[index].node.icon.file.url
            }
          />
        </IconContainer>
        {currentSlideIndex === index ? (
          <StepArrowOrange isLastElement={index === customerJourneySliderContent.edges.length - 1} />
        ) : (
          <StepArrowDark isLastElement={index === customerJourneySliderContent.edges.length - 1} />
        )}
      </ImagesContainer>
      <SlideNameContainer>
        <p>{getSlideName(customerJourneySliderContent, index)}</p>
      </SlideNameContainer>
    </SlideLabelContainer>
  )
  return CustomPaging
...
const sliderSettings = (customerJourneySliderContent, setCurrentSlideIndex, currentSlideIndex) => {
  return {
    beforeChange: (oldIndex, newIndex) => {
      setCurrentSlideIndex(newIndex)
    },
    appendDots: AppendDotsFactory(),
    customPaging: CustomPagingFactory(customerJourneySliderContent, currentSlideIndex),
    dots: true,
    speed: 500,
    slidesToShow: 1,
    slidesToScroll: 1,
...
const CustomerJourneySlider = ({ customerJournerSlider, customerJourneySliderContent }) => {
  const [currentSlideIndex, setCurrentSlideIndex] = useState(0)
  return (
    <Section>
      <StyledContainer>
        <h3>{customerJournerSlider.customerJourneySliderHeading}</h3>
        <h2>{customerJournerSlider.customerJourneySliderSubHeading}</h2>
        <SliderContainer>
