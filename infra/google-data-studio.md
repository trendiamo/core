...
GRANT SELECT ("username", "email", "first_name", "last_name", "customer_ref", "reset_password_sent_at",
...
GRANT SELECT ("created_at", "updated_at", "name", "slug", "is_affiliate") ON TABLE "accounts" TO readonly;
GRANT SELECT ("email", "role", "sender_id", "recipient_id", "account_id", "accepted_at",
              "created_at", "updated_at") ON TABLE "invites" TO readonly;

CREATE USER report WITH PASSWORD 'xxx';
GRANT readonly TO report;

CREATE VIEW users_view AS SELECT "username", "email", "first_name", "last_name", "customer_ref", "reset_password_sent_at",
              "remember_created_at", "sign_in_count", "current_sign_in_at", "last_sign_in_at", "current_sign_in_ip",
              "last_sign_in_ip", "created_at", "updated_at", "img_url", "confirmed_at", "confirmation_sent_at",
              "unconfirmed_email", "subscribed_to_newsletter", "onboarding_stage", "admin", "img_rect",
              "affiliate_role", "social_media_url", "referral_code", "referred_by_code",
...
CREATE VIEW accounts_view AS SELECT "created_at", "updated_at", "name", "slug", "is_affiliate" from accounts;
CREATE VIEW invites_view AS SELECT "email", "role", "sender_id", "recipient_id", "account_id", "accepted_at",
