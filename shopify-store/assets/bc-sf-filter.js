...
var bcSfFilterTemplate = {
    ...
    'soldOutLabelHtml': '<div class="product__label label soldout"><p class="product__label-text label__text">' + bcSfFilterConfig.label.sold_out + '</p></div>',
    'saleLabelHtml': '<div class="product__label label sale"><p class="product__label-text label__text">' + bcSfFilterConfig.label.on_sale + '</p></div>',
    'vendorHtml': '<h4 class="product__vendor h6">{{itemVendorLabel}}</h4>',

    // Grid Template
    'productGridItemHtml': '<div class="o-layout__item ' + bcSfFilterConfig.custom.grid_item_width + '">' +
                                '<a href="{{itemUrl}}" class="product-link" title="{{itemTitle}}">' +
                                    '<div class="product {{soldOutClass}} product--' + bcSfFilterConfig.custom.product_grid_align + ' ' + bcSfFilterConfig.general.product_card_shadow + '">' +
                                        '<div class="product__media u-bg-' + bcSfFilterConfig.custom.product_grid_bg + '">' +
                                           '<div class="product__img-wrapper">' +
                                            ...
                                            '{{itemLabels}}' +
                                        '</div>' +
                                        '<div class="product__details ' + bcSfFilterConfig.general.product_card_padding + '">' +
...
BCSfFilter.prototype.buildProductGridItem = function(data) {
    ...
    // Add Thumbnail
    var itemThumbUrl = images.length > 0 ? this.optimizeImage(images[0]['src'], '100x') : bcSfFilterConfig.general.no_image_url;
    var itemThumbSrc = this.getFeaturedImage(images, bcSfFilterConfig.custom.image_size);
    ...
    if(bcSfFilterConfig.custom.product_grid_second_hover && images.length > 1){
        itemFlipImage = '<div class="product__img-hover u-bg-' + bcSfFilterConfig.custom.product_grid_bg + ' lazyload js"' +
                            'data-parent-fit="cover"' +
                            'style="background-image: url(' + this.optimizeImage(images[1]['src'], '300x') + ');"></div>' +
                        '<noscript>' +
                            '<div class="product__img-hover u-bg-' + bcSfFilterConfig.custom.product_grid_bg + '" style="background-image: url(' + this.optimizeImage(images[1]['src'], '300x') + ');">' +
                            '</div>' +
                        '</noscript>';
    }
    ...
    if (!soldOut) {
        if (onSale) {
            if (priceVaries) {
                itemPriceHtml += '<span class="product__price-price"><span class="money">' + this.formatMoney(data.price_min) +'</span></span>';
                itemPriceHtml += '<strike class="product__price-cross"><span class="money">' + this.formatMoney(data.compare_at_price_min) + '</span></strike>';
            } else {
                itemPriceHtml += '<span class="product__price-price"><span class="money">' + this.formatMoney(data.price_min) +'</span></span>';
                itemPriceHtml += '<strike class="product__price-cross"><span class="money">' + this.formatMoney(data.compare_at_price_min) + '</span></strike>';
            }
        ...
        itemPriceHtml += '<span class="product__price-price product__price-price--sold">' + bcSfFilterConfig.label.sold_out + '</span>';
    }
    ...
    // Add Vendor
    var itemVendorHtml = bcSfFilterConfig.custom.product_grid_vendor ? bcSfFilterTemplate.vendorHtml.replace(/{{itemVendorLabel}}/g, data.vendor) : '';
